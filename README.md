...
```
Stage 0   GWEN Variable Selection         Geographically-weighted elastic net ranks predictors
     │
Stage 1   Correlogram Analysis            Moran's I at multiple lags → auto-detect bandwidth & block size
     │
...
### Stage 0 — GWEN Variable Selection *(optional)*

A geographically-weighted elastic net (GWEN) ranks predictor importance across space. A human checkpoint allows review before proceeding.

### Stage 1 — Correlogram Analysis

Computes Moran's I at multiple distance lags to quantify spatial autocorrelation. Automatically selects optimal bandwidth for GWR/GWRF and block size for spatial cross-validation.
...
```powershell
sparc run -p project.yml -s 0      # GWEN variable selection
sparc run -p project.yml -s 1      # Correlogram analysis
...
```
GW3C_v2.1/
├── sparc/                   # CLI entry point (sparc init / validate / run / scenario / report)
...
├── brown_uhi_run/           # Example completed UHI analysis
...
See `MANUAL.md` for the full configuration reference and `PIPELINE_GUIDE.md` for a step-by-step walkthrough.
