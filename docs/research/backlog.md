...
- [x] **E1 Extract ScenarioExecutor to sparc/scenario/executor.py** — complexity: **medium** — *Done 2026-07-02: Created `sparc/scenario/executor.py` with `ScenarioExecutor.run(actions, rollout_fn, rollout_kwargs, mode)` static method encapsulating latent rollout result-shaping. Replaced 40-line inline result-shaping block in `routes/scenarios.py::post_scenarios_chain` with a `ScenarioExecutor.run()` call. 5 new tests in `TestScenarioExecutor`; 247 passing (1 intentional failure).*
  - Files: `sparc/scenario/executor.py` (new), `sparc/server/routes/scenarios.py` (refactored chain handler), `tests/test_server_route_modules.py` (appended `TestScenarioExecutor`)
