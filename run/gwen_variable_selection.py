...
from data.data_utils import load_data, prepare_data
...
def _make_spatial_folds(coords, n_splits=5, block_size=None):
    ...
    from run.enhanced_spatial_cv import spatial_kfold_enhanced
...
def main(config_path=None, fast_mode=False):
    ...
    try:
        ...
        if gwen_cfg.get('auto_tune', True):
            from run.pipeline_paths import get_paths as _get_paths
            try:
                _paths = _get_paths()
